...
pub async fn test_redis_get_cached_weather_data(){
    ...
    let weather_data_from_redis: WeatherData = Utility::get_cached_weather_data(&test_data.name).await.unwrap();
    assert_eq!(test_data, weather_data_from_redis, "Data from redis is not equal");
...
pub async fn test_redis_delete_data(){
    ...
    let result: Option<String> = redis::cmd("GET").arg(test_data.name).query_async(&mut redis_conn).await.unwrap();
