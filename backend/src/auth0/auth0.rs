...
pub fn login(oauth2: OAuth2<Auth0>, jar: &CookieJar<'_>) -> Redirect{
    ...
    let ouath = oauth2
        .get_redirect_extras(
            ...
            &[("prompt", "login"), ("screen_hint", "signup"), ("audience", &audience)],     
            )
        .unwrap();
    //println!("{:?}", ouath);
    ouath
...
pub fn callback(code: String, state: String, token: TokenResponse<Auth0>, jar: &CookieJar<'_>) -> Result<Redirect, String> {
